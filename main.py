...
# Render requires a web server to bind to a port or it will shut down.
app = Flask(__name__)
...
# This keeps the instance from idling by hitting its own health check
async def self_ping():
    # Set this in Render Dashboard Environment Variables
    url = os.environ.get("RENDER_EXTERNAL_URL")
    if not url:
        logging.warning("RENDER_EXTERNAL_URL not set. Internal ping skipped.")
        return

    while True:
        try:
            # Pings every 10 minutes (Render's idle timeout is 15 mins)
            requests.get(url, timeout=10)
            logging.info("Ping sent to stay awake.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🚀 **Donation Bot Active**\n\nFormat: `NUMBER|MM|YYYY|CVC`", parse_mode='Markdown')
...
async def main():
    # Start Flask server in background for Render compatibility
